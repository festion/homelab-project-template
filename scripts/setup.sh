...
# Homelab Project Template Setup Script
...
check_prerequisites() {
    ...
    if [ "$node_version" -lt 16 ]; then
        log_error "Node.js version $node_version is too old. Please install Node.js 16+ and try again."
        exit 1
    fi
...
setup_project_structure() {
    ...
    # Create API structure if enabled
    if [ "$api_enabled" = "true" ] || [ "$project_type" = "api" ] || [ "$project_type" = "fullstack" ]; then
        log_info "Setting up API structure..."
        mkdir -p api/{src,routes,middleware,models,config,tests}
        
        # Create basic API template files
        create_api_template
    fi
    
    # Create dashboard structure if enabled
    if [ "$dashboard_enabled" = "true" ] || [ "$project_type" = "dashboard" ] || [ "$project_type" = "fullstack" ]; then
        log_info "Setting up dashboard structure..."
        mkdir -p dashboard/{src,public,components,pages,styles,tests}
        
        # Create basic dashboard template files
        create_dashboard_template
    fi
    
    # Create common directories
    mkdir -p {logs,config,assets,tests}
...
# Home Assistant Configuration (if using)
